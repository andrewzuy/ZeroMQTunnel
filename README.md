...
### Key File Format (Ed25519 PEM)

Real Ed25519 CURVE keypairs are generated by the script using OpenSSL:

```bash
-----BEGIN PRIVATE KEY-----
MC4CAQAwBQYDK2VwBCIEIAAAAA==<random-bytes>
-----END PRIVATE KEY-----
```

**Note**: The keys generated by `./tools/generate_keys.sh` are **real Ed25519 CURVE keypairs** compatible with the `ed25519-dalek` crate. Verify a key is valid:

```bash
openssl pkey -in server.pem -check -noout  # Should print "Key is valid"
```

The script automatically detects and replaces any random hex placeholder keys with proper Ed25519 keys.
